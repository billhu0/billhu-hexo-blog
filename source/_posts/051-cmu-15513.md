...
- Kernel offers a set of basic operations for all files
  ...
  - Look up information about a file (size, type, last modified time, ...): `stat()`, `lstat()`, `fstat()`
  - Changing the **current file position** (**seek**): `lseek()`
- Each file has a **type**
  - Regular file
    - Applications often distinguish between text and binary files. Kernel don't care. 
  - Directory
    - Consists of an array of **entries** (aka **links**). Contains at least two entries `.` and `..`
...
- Open an existing file: `open(path, flags)`
  - Flags must include exactly one of these
    - `O_RDONLY`, `O_WRONLY`, `O_RDWR`
  - Flags may also include
    - `O_APPEND`, `O_TRUNC` (delete existing contents if any), `O_CLOEXEC` (close this file if execve is called)
- Open or create a file: `open(path, flags, mode)`
  - Flags must include `O_CREAT` and exactly one of 
    - `O_WRONLY`, `O_RDWR`
  - Flags may also include
    - `O_EXCL` (fail if file does exist), `O_APPEND`, `O_TRUNC`, `O_CLOEXEC`
- Third argument must be present if `O_CREAT` appears in second argument; ignored otherwise.
...
- most general form of IO. All other IO packages are using it.
...
![](051-cmu-15513/file-sharing.png)
