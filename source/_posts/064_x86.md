...
地址总线: 8bit，但是配合H和L寄存器可以间接访问14-bit寻址空间
...
Intel 8088采用8位外部数据总线（而非8086的16位数据总线），比8086性能弱，但这样可以兼容更便宜的8位外设芯片
...
引入Protected Mode, 可以隔离用户态与内核态程序

- 保护模式有著名缺陷：一旦进入保护模式，无法通过任何软件指令切换回Real Mode，除非通过硬件或硬件中断重启CPU
...
支持Virtual 8086 mode, 可直接执行大多数8086/80286的指令，允许在保护模式下直接运行旧的DOS程序
...
支持µops (micro-operations) 内部会把x86指令转化为近似risc的简单指令
...
首次发布于1999年，首次出现在AMD Opteron系列处理器上
...
## Intel Sandy Bridge (二代酷睿)

引入AVX指令集(256-bit), 2011年随Intel Sandy Bridge发布

- 新增16个256-bit的寄存器 YMM0-YMM15, 其低128位就是原来的 XMM0-XMM15
- 支持VEX coding scheme, 三操作数模式（如 vaddpd ymm1, ymm2, ymm3）
- 仅浮点支持256-bit，整数仍仅支持128-bit
